...
parse_id_or_call input =
    getId (dropSpaces input)    `bind` (\(identifier, input1) ->
    parse_arguments input1      `bind` (\(list, input2) ->
    Just (Call identifier list, input2)
                                `fails` (\() ->
    parse_empty input1))           
                                `fails` (\() ->
    Just (Id identifier, input1)))
-- parse arguments already accounts for the things inside parenthesis (...)
{--
    getId (dropSpaces input)    `bind` (\(identifier, input1) ->
    next_symbol lparen input1   `bind` (\(_, input2) ->
    parse_arguments input2      `bind` (\(list, input3) ->
    next_symbol rparen input3   `bind` (\(_, input4) ->
    Just (Call identifier list, input4)))
                                `fails` (\() ->
    Just (Id identifier, input1))))
-}
{--
    getId (dropSpaces input)    `bind` (\ (identifier, input1) ->
    next_symbol lparen input1   `bind` (\ (_, input2) ->
    parse_arguments input2      `bind` (\ (args, input3) ->
    next_symbol rparen input3   `bind` (\ (_, input4) ->
    Just (Call identifier args, input4))
                                `fails` (\() ->
    Just (Id identifier, input1)))))
    -- *** Stub - replace this with code ***
-}
...
parse_arguments input =
    ...
    parse_E input1                  `bind` (\ (expr, input2) ->
    parse_argtail input2            `bind` (\ (argtail, input3) ->
    next_symbol rparen input3       `bind` (\ (_, input4) ->
    Just (expr:argtail, input4)))))
    -- *** Stub - replace this with code ***
...
parse_argtail input = 
    ...
    parse_E input1                  `bind` (\ (expr, input2) ->
    parse_argtail input2            `bind` (\ (atail, input3) ->
    Just (expr:atail, input3))))
    -- *** Stub - replace this with code ***
--
