...
unify1 problems = 
    do
    let 
        firstProblem = (fst problems) !! 0
        es1 = fst firstProblem
        exp1 = snd firstProblem
        sub = snd problems
        problemList = drop 1 (fst problems)
    if ifId es1 || ifId exp1 || ifVar es1 || ifVar exp1 then -- step 1
        (if es1 == exp1 then unify1 (problemList, sub)
        else Just problems)
    else -- step 2 and above
        Just problems
